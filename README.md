...
- **本地历史与导出目录**：评分记录保存在项目的 `data/assessment-history/`（不提交到 Git）。在 Chromium 系浏览器中可选择一个本地文件夹，导出的 JSON 报告会直接写入其中；不支持该能力的浏览器会改为下载文件。
...
| `GET/DELETE /api/writing/history` | 读取或删除本地评分历史 |
...
本项目没有用户系统或云数据库。作文内容会发送给你选择的模型服务以生成反馈；服务端评分历史仅落在项目本地目录，浏览器设置仅保存在当前设备的浏览器中。请根据自己的隐私要求部署和使用。
